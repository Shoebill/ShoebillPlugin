...
void NativeFunctionManager::registerFunction(AMX* amx, std::string name, AMX_NATIVE functionAddr, int number)
{
	functions[name] = functionAddr;
	overridePointer(amx, functionAddr, name.c_str());
}

AMX_NATIVE NativeFunctionManager::findFunction(std::string name)
{
	auto it = functions.find(name);
	return it != functions.end() ? it->second : nullptr;
}

std::map<std::string, hookedNative*> natives;
...
static cell AMX_NATIVE_CALL n_OnAmxShowPlayerDialog(AMX* amx, cell* params)
{
	...
	int ret = hookedNative->originalFunc(amx, params);
	hookedNative->hook->get()->hook();
	char caption[64], info[1024], button1[32], button2[32];
...
void NativeFunctionManager::overridePointer(AMX* amx, AMX_NATIVE function, const char *name)
{
	for (int i = 0; i < sizeof(FunctionNatives) / sizeof(AMX_NATIVE_INFO); i++)
	{
		if (!strcmp(name, FunctionNatives[i].name))
		{
			...
			if (it == natives.end())
			{
				...
				hook->hook->get()->init(function, FunctionNatives[i].func);
				hook->hook->get()->hook();
...
{
	typedef std::map<std::string, hookedNative*>::iterator it_type;
	for (it_type iterator = natives.begin(); iterator != natives.end(); iterator++)
	{
		iterator->second->hook->get()->unhook();
	}
	natives.clear();
}