		 ...
		 jint materialcolor)
{
	...
	char _txdname[1024];
	wcs2mbs((unsigned int) getServerCodepage(), wmsg, len, _txdname, sizeof(_txdname));
	...
	wcs2mbs((unsigned int) getServerCodepage(), wmsg2, len, _texturename, sizeof(_texturename));
		 ...
		 jstring texturename, jint materialcolor)
{
	...
	char _texturename[1024];
	wcs2mbs((unsigned int) getServerCodepage(), wmsg, len, _texturename, sizeof(_texturename));
		 ...
		 jint fontsize, jint bold, jint fontcolor, jint backcolor, jint textalignment)
{
	...
	char _text[1024];
	wcs2mbs((unsigned int) getServerCodepage(), wmsg, len, _text, sizeof(_text));
	...
	char _fontface[1024];
	wcs2mbs((unsigned int) getServerCodepage(), wmsg2, len, _fontface, sizeof(_fontface));
	env->ReleaseStringChars(fontface, wmsg2);

	SetObjectMaterialText(objectid, _text, materialindex, materialsize, _fontface, fontsize, (bool) bold, fontcolor,
		 ...
		 jstring fontface, jint fontsize, jint bold, jint fontcolor, jint backcolor, jint textalignment)
{
	...
	char _text[1024];
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg, len, _text, sizeof(_text));
	env->ReleaseStringChars(text, wmsg);
	...
	char _fontface[1024];
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg2, len, _fontface, sizeof(_fontface));
	env->ReleaseStringChars(fontface, wmsg2);

	SetPlayerObjectMaterialText(playerid, objectid, _text, materialindex, materialsize, _fontface, fontsize,
		...
		(JNIEnv *env, jclass, jint playerid)
{
	...
	auto len = mbs2wcs((unsigned int) getServerCodepage(), name, -1, wstr, sizeof(wstr) / sizeof(wstr[0]));
		 ...
		 jint usepos)
{
	...
	char str[1024];
	wcs2mbs((unsigned int) getServerCodepage(), wurl, len, str, sizeof(str));
		...
		(JNIEnv *env, jclass, jint playerid, jstring name)
{
	...
	char str[1024];
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg, len, str, sizeof(str));
	env->ReleaseStringChars(name, wmsg);
		...
		(JNIEnv *env, jclass, jint playerid, jint textid, jstring string)
{
	...
	char _string[1024];
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg, len, _string, sizeof(_string));
		...
		(JNIEnv *env, jclass, jint playerid, jint color, jstring message)
{
	...
	char msg[1024];
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg, len, msg, sizeof(msg));
		...
		(JNIEnv *env, jclass, jint color, jstring message)
{
	...
	char msg[1024];
	wcs2mbs((unsigned int) getServerCodepage(), wmsg, len, msg, sizeof(msg));
	env->ReleaseStringChars(message, wmsg);
		...
		(JNIEnv *env, jclass, jstring varname)
{
	...
	jchar wstr[256];
	auto len = mbs2wcs((unsigned int) getServerCodepage(), var, -1, wstr, sizeof(wstr) / sizeof(wstr[0]));
	return env->NewString(wstr, len);
}
		...
		(JNIEnv *env, jclass, jint playerid)
{
	...
	jchar wstr[2048];
	auto len = mbs2wcs((unsigned int) getServerCodepage(), retstr, -1, wstr, sizeof(wstr) / sizeof(wstr[0]));
		 ...
		 jfloat drawDistance, jint attachedplayerid, jint attachedvehicleid, jboolean testLOS)
{
	...
	return CreatePlayer3DTextLabel(playerid, str, color, x, y, z, drawDistance, attachedplayerid, attachedvehicleid,
	                               testLOS);
}
		 ...
		 jstring caption, jstring info, jstring button1, jstring button2)
{
	...
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg1, len, str_caption, sizeof(str_caption));
	env->ReleaseStringChars(caption, wmsg1);

	auto wmsg2 = env->GetStringChars(info, NULL);
	len = env->GetStringLength(info);
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg2, len, str_info, sizeof(str_info));
	env->ReleaseStringChars(info, wmsg2);

	auto wmsg3 = env->GetStringChars(button1, NULL);
	len = env->GetStringLength(button1);
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg3, len, str_button1, sizeof(str_button1));
	env->ReleaseStringChars(button1, wmsg3);

	auto wmsg4 = env->GetStringChars(button2, NULL);
	len = env->GetStringLength(button2);
	wcs2mbs((unsigned int) getPlayerCodepage(playerid), wmsg4, len, str_button2, sizeof(str_button2));
                                                                                  ...
                                                                                  jobjectArray args)
{
	...
	for (auto i = 0; i < arrayLength; i++)
	{
		...
		if (mid)
		{
			...
			else if (!strcmp(classname, "class net.gtaun.shoebill.amx.types.ReferenceInt") ||
			         !strcmp(classname, "class net.gtaun.shoebill.amx.types.ReferenceFloat"))
			{
				cell amx_addr, *phys_addr;

				amx_Allot(amx, 1, &amx_addr, &phys_addr);
				params[i + 1] = amx_addr;
				references[std::pair<jobject, std::string>(object, std::string(classname))] = std::pair<cell *, cell>(
						phys_addr, amx_addr);
			}
		...
		(JNIEnv *env, jclass, jint pAmx, jint idx, jint returnType, jobjectArray args)
{
	...
	for (auto i = arrayLength - 1; i >= 0; i--)
	{
		...
		if (mid)
		{
			...
			{
				pushJavaInteger(env, amx, object);
			}
			else if (className == "class java.lang.Float")
			{
				pushJavaFloat(env, amx, object);
			}
			else if (className == "class net.gtaun.shoebill.amx.types.ReferenceInt" ||
			         className == "class net.gtaun.shoebill.amx.types.ReferenceFloat")
			{
				pushJavaReferenceFloatInt(env, amx, object, references, className);
			}
			else if (className == "class net.gtaun.shoebill.amx.types.ReferenceString")
			{
				pushJavaReferenceString(env, amx, object, references, className);
			}
			else if (className.find("class [") != std::string::npos) //Array
			{
				...
				else if (className == "class [Ljava.lang.String;") //If String Array is present
				{
					...
					for (int a = 0; a < javaArrayLength; a++)
					{
						...
						char text[1024];
						wcs2mbs((unsigned int)getServerCodepage(), wmsg, len, text, sizeof(text));
						env->ReleaseStringChars(jstr, wmsg);
...
JNIEXPORT void JNICALL Java_net_gtaun_shoebill_SampNativeFunction_setPVarInt(JNIEnv *env, jclass jclass1, jint playerid,
...
JNIEXPORT jint JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getPVarInt(JNIEnv *env, jclass jclass1, jint playerid,
                                                                             jstring name)
{
	auto varName = env->GetStringUTFChars(name, NULL);
...
JNIEXPORT void JNICALL Java_net_gtaun_shoebill_SampNativeFunction_setPVarString(JNIEnv *env, jclass jclass1,
...
JNIEXPORT jstring JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getPVarString(JNIEnv *env, jclass jclass1,
                                                                                   jint playerid,
                                                                                   jstring name)
{
	auto varName = env->GetStringUTFChars(name, NULL);
...
JNIEXPORT void JNICALL Java_net_gtaun_shoebill_SampNativeFunction_setPVarFloat(JNIEnv *env, jclass jclass1,
                                                                               jint playerid, jstring name,
                                                                               jfloat value)
{
	auto varName = env->GetStringUTFChars(name, NULL);
...
JNIEXPORT jfloat JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getPVarFloat(JNIEnv *env, jclass jclass1,
                                                                                 jint playerid,
                                                                                 jstring name)
{
	auto varName = env->GetStringUTFChars(name, NULL);
...
JNIEXPORT jint JNICALL Java_net_gtaun_shoebill_SampNativeFunction_deletePVar(JNIEnv *env, jclass jclass1, jint playerid,
                                                                             jstring name)
{
	auto varName = env->GetStringUTFChars(name, NULL);
...
JNIEXPORT jint JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getPVarsUpperIndex(JNIEnv *env, jclass jclass1,
                                                                                     jint playerid)
{
	return GetPVarsUpperIndex(playerid);
}

JNIEXPORT jstring JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getPVarNameAtIndex(JNIEnv *env, jclass jclass1,
                                                                                        jint playerid,
                                                                                        jint index)
{
	char varName[1024];
...
JNIEXPORT jint JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getPVarType(JNIEnv *env, jclass jclass1,
                                                                              jint playerid, jstring name)
{
	auto varName = env->GetStringUTFChars(name, NULL);
...
JNIEXPORT jint JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getConsoleVarAsInt(JNIEnv *env, jclass jclass1,
                                                                                     jstring name)
{
	auto varName = env->GetStringUTFChars(name, NULL);
...
JNIEXPORT jboolean JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getConsoleVarAsBool(JNIEnv *env, jclass jclass1,
                                                                                          jstring name)
{
	auto varName = env->GetStringUTFChars(name, NULL);
...
JNIEXPORT jstring JNICALL Java_net_gtaun_shoebill_SampNativeFunction_getConsoleVarAsString(JNIEnv *env, jclass jclass1,
                                                                                           jstring name)
{
	auto varName = env->GetStringUTFChars(name, NULL);
	char result[1024];
